...
public class ProductController {
    ...
    @GetMapping
    public ResponseEntity<PageResponse<ProductOutDtoMIni>> getAllAvailable(
    ...
    @GetMapping("/all")
    public ResponseEntity<PageResponse<ProductOutDtoMIni>> getAll(
            @RequestParam(defaultValue = "0", required = false) int page,
            @RequestParam(defaultValue = "10", required = false) int size) {
    ...
    @GetMapping("/search")
    public ResponseEntity<PageResponse<ProductOutDtoMIni>> getSearch(
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ProductOutDto> update(@PathVariable Long id,
    ...
    public ResponseEntity<ProductOutDto> updateStatus(@PathVariable Long id,
                                                      @RequestBody ProductStatusInDto status) {

        return ResponseEntity.ok(productMapped.toDto(
                productService.updateStatus(id, productMapped.toModelStatus(status))));
    }
